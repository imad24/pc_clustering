...
def Cluster_series_plot(data_df,cluster_df,centroid_only= False,headers = None,tick_frequency = 3,top = None):
    
    list_it = list(range(len(data_df.columns)))
    
    
    clusters = list(set(cluster_df['Cluster']))
    if top: clusters = np.random.randint(1,len(clusters),size= top)
    nc = len(clusters) if top is None else top
